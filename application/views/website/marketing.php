 <div class="container mb-4">
    <h2 class="h2 display-4 text-center mb-4" style="padding-top: 4rem">Marketing Digital</h2>
    <div class="row">

      <!-- Start Card-->
      <div class="col-md-4">
        <div class="single-price">
          <div class="price-head">
            <h3>Básico</h3>
            <p>$100.000 <!--<pan>anual</span>--></p>
          </div>

          <div class="price-content">
            <ul>
              <li><i class="fa fa-check-circle"></i>Manejo de redes sociales</li>
              <li><i class="fa fa-check-circle"></i>Publicaciones</li>
              <li><i class="fa fa-check-circle"></i>Diseño Grafico</li>
              <li><i class="fa fa-times-circle"></i>Google Ads</li>
              <li><i class="fa fa-times-circle"></i>Fotografia</li>
              <li><i class="fa fa-times-circle"></i>Seo</li>
              <li><i class="fa fa-times-circle"></i>Community Manager</li>
            </ul>
          </div>
        </div>
      </div>
      <!-- Finish Card-->

      <!-- Start Card-->
      <div class="col-md-4">
        <div class="single-price">
          <div class="price-head">
            <h3>Medio</h3>
            <p>$200.000 <!--<pan>anual</span>--></p>
          </div>

          <div class="price-content">
            <ul>
              <li><i class="fa fa-check-circle"></i>Manejo de redes sociales</li>
              <li><i class="fa fa-check-circle"></i>Publicaciones</li>
              <li><i class="fa fa-check-circle"></i>Diseño Grafico</li>
              <li><i class="fa fa-check-circle"></i>Google Ads</li>
              <li><i class="fa fa-check-circle"></i>Fotografia</li>
              <li><i class="fa fa-times-circle"></i>Seo</li>
              <li><i class="fa fa-times-circle"></i>Community Manager</li>
            </ul>
          </div>
        </div>
      </div>
      <!-- Finish Card-->

      <!-- Start Card-->
      <div class="col-md-4">
        <div class="single-price">
          <div class="price-head">
            <h3>Full</h3>
            <p>$300.000 <!--<pan>anual</span>--></p>
          </div>

          <div class="price-content">
            <ul>
              <li><i class="fa fa-check-circle"></i>Manejo de redes sociales</li>
              <li><i class="fa fa-check-circle"></i>Publicaciones</li>
              <li><i class="fa fa-check-circle"></i>Diseño Grafico</li>
              <li><i class="fa fa-check-circle"></i>Google Ads</li>
              <li><i class="fa fa-check-circle"></i>Fotografia</li>
              <li><i class="fa fa-check-circle"></i>Seo</li>
              <li><i class="fa fa-check-circle"></i>Community Manager</li>
            </ul>
          </div>
        </div>
      </div>
      <!-- Finish Card-->

    </div>
  </div>